...
for k = 1:length(strs)
    ...
    if exist(sprintf('F:\\%s\\data_set_FRFT_0.5\\T%s', distance, str))==0 %%判断文件夹是否存在
        mkdir(sprintf('F:\\%s\\data_set_FRFT_0.5\\T%s', distance, str));  %%不存在时候，创建文件夹
    end
    ...
    for j=1:LengthFiles
        ...
        disp('packing pic...');
        for i = 0:(size(signal)/N)-1
            signal_data = signal((i*N+1):((i+1)*N));
%             [S, f, t] = spectrogram(signal_data, h, noverlap, N, fs);
            ...
            pic_name = sprintf('F:\\%s\\data_set_256_256\\T%s\\%d.png', distance, str, index + i);
